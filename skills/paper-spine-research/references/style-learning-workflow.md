...
4. Create `paper_rewriting_output/style_metrics.md` from local text statistics
   such as section counts, paragraph counts, typical paragraph length, citation
   density, figure/table callout density, and common opening/closing moves. If
   a bundled metrics script is added in the future, use it only after confirming
   the active script exists on disk.
