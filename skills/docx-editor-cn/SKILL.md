...
- **Official templates are base documents**: When the user provides a school/teacher template, build from that `.docx` or merge the new body into it. Preserve the native cover, section breaks, page setup, headers/footers, styles, numbering, and real TOC fields. Do not hand-recreate the cover unless the template cannot be used.
